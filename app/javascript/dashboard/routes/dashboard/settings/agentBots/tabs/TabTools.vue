...
const TOOLS = [
  ...
  {
    key: 'get_marketing_campaigns',
    icon: '🎯',
    hasParams: false,
  },
  {
    key: 'get_kb_resources',
    icon: '📚',
    hasParams: false,
  },
  {
...
const getToolConfig = key => props.form.agent_behavior_config.tools[key] || {};
...
<template>
  <div class="flex flex-col gap-0 border border-n-weak rounded-xl overflow-hidden divide-y divide-n-weak">
    <div v-for="tool in TOOLS" :key="tool.key">
      <div class="p-4">
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div class="flex flex-col gap-0.5 min-w-0">
              <span class="text-sm font-medium text-n-slate-12">
                {{ tool.icon }} {{ $t(`AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.${tool.key}.LABEL`) }}
              </span>
              <span class="text-xs text-n-slate-10 font-mono">{{ tool.key }}_tool</span>
          ...
          >
            <span>{{ expanded[tool.key] ? $t('AGENT_BOTS.CONFIG.TOOLS.COLLAPSE') : $t('AGENT_BOTS.CONFIG.TOOLS.EXPAND') }}</span>
            <span class="i-lucide-chevron-down transition-transform" :class="{ 'rotate-180': expanded[tool.key] }" />
          </button>
        </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="flex flex-col gap-3">
              ...
              <template v-else-if="tool.key === 'get_all_products'">
                <div class="flex flex-col gap-1">
                  <label class="text-xs text-n-slate-11">{{ $t('AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.get_all_products.MODE_LABEL') }}</label>
                  <label class="flex items-center gap-2 text-xs text-n-slate-12 cursor-pointer">
                    <input
                      type="radio"
                      value="all"
                      :checked="getToolConfig(tool.key).listing_mode === 'all'"
                      class="accent-n-brand"
                      @change="getToolConfig(tool.key).listing_mode = 'all'"
                    />
                    {{ $t('AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.get_all_products.MODE_ALL') }}
                  </label>
                  <div class="flex items-center gap-2">
                    <input
                      type="radio"
                      value="limited"
                      :checked="getToolConfig(tool.key).listing_mode === 'limited'"
                      class="accent-n-brand"
                      @change="getToolConfig(tool.key).listing_mode = 'limited'"
                    />
                    <span class="text-xs text-n-slate-12">{{ $t('AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.get_all_products.MODE_LIMITED') }}</span>
                    <input
                      v-if="getToolConfig(tool.key).listing_mode === 'limited'"
                      type="number"
                  ...
                  <label class="flex items-center gap-2 text-xs text-n-slate-12 cursor-pointer">
                    <input
                      type="radio"
                      value="summary"
                      :checked="getToolConfig(tool.key).listing_mode === 'summary'"
                      class="accent-n-brand"
                      @change="getToolConfig(tool.key).listing_mode = 'summary'"
                    />
                    {{ $t('AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.get_all_products.MODE_SUMMARY') }}
                  </label>
                </div>
                <label class="flex items-center gap-2 text-xs text-n-slate-12 cursor-pointer">
                  <input
              ...
              <template v-else-if="tool.key === 'send_document'">
                <div class="flex flex-col gap-1">
                  <label class="text-xs text-n-slate-11">{{ $t('AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.send_document.SOURCE_LABEL') }}</label>
                  <label class="flex items-center gap-2 text-xs text-n-slate-12 cursor-pointer">
                    <input
                      type="radio"
                      value="catalog"
                      :checked="(getToolConfig(tool.key).sources || ['catalog']).includes('catalog') && !(getToolConfig(tool.key).sources || []).includes('nauto_api')"
                      class="accent-n-brand"
                      @change="getToolConfig(tool.key).sources = ['catalog']"
                    />
                    {{ $t('AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.send_document.SOURCE_CATALOG') }}
                  </label>
                  <label class="flex items-center gap-2 text-xs text-n-slate-12 cursor-pointer">
                    <input
                      type="radio"
                      value="both"
                      :checked="(getToolConfig(tool.key).sources || []).includes('nauto_api')"
                      class="accent-n-brand"
                      @change="getToolConfig(tool.key).sources = ['catalog', 'nauto_api']"
                    />
                    {{ $t('AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.send_document.SOURCE_API') }}
                  </label>
                </div>
                <div class="flex flex-col gap-1">
                  <label class="text-xs text-n-slate-11">{{ $t('AGENT_BOTS.CONFIG.TOOLS.TOOLS_LIST.send_document.TYPES_LABEL') }}</label>
                  <label v-for="type in ['pdf', 'image', 'video']" :key="type" class="flex items-center gap-2 text-xs text-n-slate-12 cursor-pointer">
                    <input
                      type="checkbox"
            ...
            <div class="flex flex-col gap-2">
              <div class="flex items-center gap-1">
                ...
                <span class="text-xs text-n-slate-9">{{ $t('AGENT_BOTS.CONFIG.TOOLS.EXAMPLES_MAX') }}</span>
              </div>
              <p class="text-xs text-n-slate-10">{{ $t('AGENT_BOTS.CONFIG.TOOLS.EXAMPLES_HINT') }}</p>
