...
class Api::V1::Accounts::InboxReengagementTemplatesController < Api::V1::Accounts::BaseController
  before_action :fetch_inbox
  before_action :validate_whatsapp_channel

  def show
    service = ReengagementTemplateManagementService.new(@inbox)
  ...
  def validate_whatsapp_channel
    ...
    render json: { error: 'Reengagement template operations only available for WhatsApp channels' },
           status: :bad_request
  end

  def extract_template_params
    params.require(:template).permit(:message, :language)
  end
