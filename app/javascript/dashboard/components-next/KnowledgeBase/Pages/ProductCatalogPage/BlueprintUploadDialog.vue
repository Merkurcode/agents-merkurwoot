<template>
  <div class="w-[30rem] z-50 absolute top-10 right-0 bg-n-alpha-3 backdrop-blur-[100px] p-6 rounded-xl border border-n-weak shadow-md flex flex-col gap-6">
    <div class="flex items-start justify-between">
      <div>
        <div class="flex items-center gap-2 mb-1">
          <i class="i-lucide-bot w-5 h-5 text-n-blue-11" />
          <h3 class="text-base font-medium text-n-slate-12">
            {{ $t('KNOWLEDGE_BASE.PRODUCT_CATALOG.BLUEPRINT.UPLOAD.TITLE') }}
          </h3>
        </div>
        <p class="text-sm text-n-slate-11 mt-1">
          {{ $t('KNOWLEDGE_BASE.PRODUCT_CATALOG.BLUEPRINT.UPLOAD.DESCRIPTION') }}
        </p>
      </div>
      <button
    ...
    <div class="flex flex-col gap-4">
      ...
      >
        ...
        <div v-else class="flex items-center gap-3 min-w-0">
          ...
          <div class="flex-1 text-left min-w-0">
            ...
            <p class="text-xs text-n-slate-11">{{ formatFileSize(selectedFile.size) }}</p>
          </div>
          <button
      ...
      <div v-if="isUploading || uploadStatus" class="space-y-2">
        ...
        <div v-if="uploadStatus === 'success'" class="bg-n-green-2 border border-n-green-6 rounded-lg p-3">
          <p class="text-xs text-n-green-11">
            {{ $t('KNOWLEDGE_BASE.PRODUCT_CATALOG.BLUEPRINT.UPLOAD.SUCCESS_MESSAGE') }}
          </p>
        </div>
      ...
      <div class="flex gap-3">
        ...
        />
        <Button
          :label="$t('KNOWLEDGE_BASE.PRODUCT_CATALOG.BLUEPRINT.UPLOAD.UPLOAD_BTN')"
...
const statusMessage = computed(() => {
  if (uploadStatus.value === 'processing') return t('KNOWLEDGE_BASE.PRODUCT_CATALOG.UPLOAD.PROCESSING');
  if (uploadStatus.value === 'success') return t('KNOWLEDGE_BASE.PRODUCT_CATALOG.UPLOAD.COMPLETED');
  if (uploadStatus.value === 'error') return t('KNOWLEDGE_BASE.PRODUCT_CATALOG.UPLOAD.FAILED');
...
const handleUpload = async () => {
  ...
  try {
    ...
    const response = await store.dispatch('productCatalogs/blueprintUpload', selectedFile.value);
