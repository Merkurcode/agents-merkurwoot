  ...
  module Hubspot
    ...
    class ProcessorService < Crm::BaseProcessorService
      ...
      def create_event(params)
        ...
        if response && response['id'].present?
          event_id = response['id'].to_s
          appointment.store_external_id('hubspot', event_id) if appointment
          Rails.logger.info "Meeting created in HubSpot: #{event_id}"
          { success: true, event_id: event_id }
        else
          { success: false, error: 'Failed to create meeting', response: response }
      ...
      def store_external_id(contact, hubspot_id)
        contact.additional_attributes ||= {}
        contact.additional_attributes['external'] ||= {}
        contact.additional_attributes['external']['hubspot_lead_id'] = hubspot_id
        contact.save(validate: false)
      end
...
>>>>>>> 0a894911e (feat: Add HubSpot CRM integration)
    end
  end
end
