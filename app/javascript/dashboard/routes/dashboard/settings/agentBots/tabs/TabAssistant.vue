...
const PRESETS = {
  ...
  lala: {
    ...
    speaking_style: '¡Claro que sí! Para una familia de 4, te recomendaría el Corolla Cross. ¿Tienes niños pequeños?',
  ...
  sol: {
    ...
    speaking_style: '¡Esto es justo lo que buscabas! Mira qué increíble opción tenemos para ti.',
  ...
  nova: {
    ...
    speaking_style: 'Entendido. Para ese perfil te recomiendo el modelo A o el B. ¿Cuál te interesa?',
...
const presetOptions = [
  ...
  { value: 'custom', label: t('AGENT_BOTS.CONFIG.ASSISTANT.PRESETS.custom') },
];
</script>
...
<template>
  <div class="flex flex-col gap-6">
    <div class="flex flex-col gap-2">
      ...
      <p class="text-xs text-n-slate-11">{{ $t('AGENT_BOTS.CONFIG.ASSISTANT.PRESET_HINT') }}</p>
    </div>
    ...
    <TextArea
      ...
      :placeholder="$t('AGENT_BOTS.CONFIG.ASSISTANT.SPEAKING_STYLE_PLACEHOLDER')"
      :rows="3"
    />
    ...
    <div class="flex flex-col gap-1">
      <TextArea
        ...
        :placeholder="$t('AGENT_BOTS.CONFIG.ASSISTANT.INSPIRATION_PLACEHOLDER')"
        :rows="2"
      />
      <p class="text-xs text-n-slate-11">{{ $t('AGENT_BOTS.CONFIG.ASSISTANT.INSPIRATION_HINT') }}</p>
    </div>
  </div>
</template>
