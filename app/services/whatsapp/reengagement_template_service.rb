...
class Whatsapp::ReengagementTemplateService
  DEFAULT_LANGUAGE = 'es_MX'
  WHATSAPP_API_VERSION = 'v14.0'
  TEMPLATE_CATEGORY = 'MARKETING'
  TEMPLATE_STATUS_PENDING = 'PENDING'
  ...
  def generate_template_name(base_name)
    ...
    Whatsapp::ReengagementTemplateNameService.generate_next_template_name(
      base_name, @whatsapp_channel.inbox.id, current_template_name
    )
  end

  def current_template_name_from_config
    @whatsapp_channel.inbox.reengagement_config&.dig('template', 'name')
  end
  ...
  def process_template_creation_response(response, template_config = {})
    if response.success?
      ...
      Rails.logger.error "WhatsApp reengagement template creation failed: #{response.code} - #{response.body}"
      {
        success: false,
