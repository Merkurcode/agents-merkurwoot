class ProductCatalogs::RateLimiterService
  ...
  class << self
    ...
    def acquire_validation_lock(account_id, suffix = nil)
      lock_key = build_validation_lock_key(account_id, suffix)
    ...
    def release_validation_lock(account_id, suffix = nil)
      lock_key = build_validation_lock_key(account_id, suffix)
    ...
    def acquire_lock(account_id, operation_type, suffix = nil)
      lock_key = build_lock_key(account_id, suffix)
    ...
    def lock_info(account_id)
      ...
      $alfred.with do |redis|
...

        {
          ...
          started_at: Time.zone.at(timestamp.to_i),
    ...
    def build_lock_key(account_id, suffix = nil)
      base = "#{LOCK_KEY_PREFIX}:#{account_id}"
      suffix ? "#{base}:#{suffix}" : base
    end

    def build_validation_lock_key(account_id, suffix = nil)
      base = "#{VALIDATION_LOCK_KEY_PREFIX}:#{account_id}"
      suffix ? "#{base}:#{suffix}" : base
    end
